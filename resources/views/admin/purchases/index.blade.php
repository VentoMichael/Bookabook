...
@section('content')
    ...
    @if(count($books))
        ...
        @foreach($books as $key => $value)
            <section class="containerPrincipalMeat">
                @if($value != $oldBook['academic_years'])
                    <h3 aria-level="3"
                            ...
                            <section class="mb-12 mt-4">
                                ...
                                <div class="mb-8">
                                    <div class="sm:flex flex-col">
                                        <img role="img"
                                             aria-label="Photo de couverture de {{$v->title}}"
                                             class="sm:max-w-xs"
                                             src="{{ asset('storage/'.$v->picture) }}"
                                             alt="">
                                        <div class="mt-4">
                                            <p class="text-xl">{{$v->title}}</p>
                                            <p class="text-md">{{$v->total_quantity}}
                                                commandes</p>
                                        </div>
                                    </div>
                                    <form
                                        aria-label="Envoie de notification pour {{$v->title}}"
                                        role="form"
                                        method="POST"
                                        action="{{ route('purchases.sendNotif')}}">
                                        @csrf
                                        @method('PUT')
                                        <input type="hidden" value="{{$v->title}}" name="bookTitle">
